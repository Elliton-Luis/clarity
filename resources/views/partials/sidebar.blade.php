<aside class="w-64 h-screen bg-white shadow-lg fixed top-0 left-0 overflow-y-auto
    ...
    <div class="px-4 my-4">
        <a href="{{ route('tasks.create') }}" class="flex items-center justify-center w-full px-5 py-3 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700 transition-colors">
    ...
    <nav class="mt-4">
        <ul>
            <li class="px-4">
                <a href="{{route('tasks.index')}}" class="flex items-center px-4 py-3 rounded-md transition-colors
                            {{ Request::is('tasks*') && !Request::is('lixeira*') ?
            ...
            <li class="px-4 mt-2">
                <a href="{{ route('tasks.trash') }}" class="flex items-center px-4 py-3 rounded-md transition-colors
                            {{ Request::is('lixeira*') ?
                                'bg-stone-100 text-indigo-600 font-semibold' :
                                'text-stone-600 hover:bg-stone-50 hover:text-stone-900' }}">
    ...
    <div class="absolute bottom-0 left-0 w-full border-t border-stone-200">
        @auth
            <div class="p-4">
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <svg class="h-6 w-6 text-stone-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                    </div>
                    <div class="ml-3">
        ...
        <div class="p-4 border-t border-stone-200">
            <form method="POST" action="{{ route('logout') }}" class="w-full">
                @csrf
                <button type="submit" class="flex items-center text-stone-600 hover:text-stone-900 w-full">
